 ...
 * ```
 */
export function hasTypeIn(value, types) {
 ...
 * ```
 */
export function isEqType(value1, value2) {
 ...
 * ```
 */
export function isEqValue(value1, value2) {
    // 判断是否是相同类型
    if (!isEqType(value1, value2))
        return false;
    if (isSimpleType(value1) || isSimpleType(value1))
